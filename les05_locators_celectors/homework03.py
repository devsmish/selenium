...
def test_menu_prog(driver):
    prog_button = driver.find_element(
        By.CSS_SELECTOR, 'a.tn-atom[href="#submenu:more"]'
    )
...
def test_menu_payment(driver):
    payment_methods = driver.find_element(
        By.CSS_SELECTOR, 'a.tn-atom[href="#rec1921734713"]'
    )
...
def test_menu_blog(driver):
    blog = driver.find_element(
        By.CSS_SELECTOR, 'a.tn-atom[href="https://blog.itcareerhub.de/"]'
    )
...
def test_button_callback(driver):
    ...
    time.sleep(2)
    popup_title = driver.find_element(
        By.CSS_SELECTOR, 'div[field="tn_text_175871291756015470"]'
    )
    assert "Запишитесь на бесплатную" in popup_title.get_attribute("textContent")


def test_language_switch_to_de_to_ru(driver):
    lang_de_btn = driver.find_element(
        By.CSS_SELECTOR, 'a[href="/"][style*="color: inherit"]'
    )
    lang_de_btn.click()
    time.sleep(3)
    assert "/ru" not in driver.current_url

    lang_ru_btn = driver.find_element(
        By.CSS_SELECTOR, 'a[href="/ru"][style*="color: inherit"]'
    )
    lang_ru_btn.click()
    time.sleep(3)
